...
#
# Bars Format:
# Date,Open,High,Low,Close,Volume,Adj Close,Extra Datas
#
...
class RowParser(csvfeed.RowParser):
    ...
    def parseBar(self, csvRowDict):
        ...
        return self.__barClass(dateTime, open_, high, low, close, volume, adjClose, self.__frequency, extra=extra)
...
class Feed(csvfeed.BarFeed):
    ...
    .. note::
        My Finance csv files lack timezone information.
    ...
    def dispatch(self):
        # 先走父类：发出 bar 事件，触发 strategy.onBars()
        ret = super(Feed, self).dispatch()
        # 在同一根 bar 内立即用当前 bar 填充 pending orders
        if self.__broker is not None:
            bars = self.getCurrentBars()
            if bars is not None:
                self.__broker.fillPendingOrders(bars)
        return ret
