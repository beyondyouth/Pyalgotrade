...
class TutorialsTestCase(common.TestCase):
    def testTutorial1(self):
        with common.CopyFiles(
            ...
            self.assertEqual(
                common.head_file("tutorial-1.output", 3), res.get_output_lines(True)[:3]
            )
            self.assertEqual(
...
"""
            ...
            lines = res.get_output_lines(True)
            self.assertEqual(common.head_file("csvfeed_1.output", 10), lines[0:10])
            self.assertEqual(common.tail_file("csvfeed_1.output", 10), lines[-10:])
            self.assertTrue(res.exit_ok())
...
class CompInvTestCase(common.TestCase):
    def testCompInv_1(self):
        ...
        with common.CopyFiles(files, "."):
            ...
            self.assertEqual(lines, common.head_file("compinv-1.output", len(lines)))


class StratAnalyzerTestCase(common.TestCase):
    def testSampleStrategyAnalyzer(self):
        with common.CopyFiles(
            [os.path.join("testcases", "data", "orcl-2000-yahoofinance.csv")], "."
        ):
            ...
            self.assertEqual(
                lines, common.head_file("sample-strategy-analyzer.output", len(lines))
            )
...
class TechnicalTestCase(common.TestCase):
    def testTechnical_1(self):
        ...
        lines = res.get_output_lines()
        self.assertEqual(lines, common.head_file("technical-1.output", len(lines)))
...
"""
            ...
            self.assertTrue(res.exit_ok())
            self.assertEqual(
                res.get_output_lines()[-10:], common.tail_file("bbands.output", 10)
            )
...
"""
            ...
            self.assertTrue(res.exit_ok())
            self.assertEqual(
                res.get_output_lines()[-10:], common.tail_file(expected_output, 10)
            )
